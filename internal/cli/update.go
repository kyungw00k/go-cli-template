...
var updateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		assetName := fmt.Sprintf("{{PROJECT_NAME}}_%s_%s", runtime.GOOS, runtime.GOARCH)
		...
		tmpDir, err := os.MkdirTemp("", "{{PROJECT_NAME}}-update-*")
		if err != nil {
			return err
		}
		...
		newBin := filepath.Join(tmpDir, "{{PROJECT_NAME}}")
		backup := exe + ".bak"
...
func fetchLatestRelease(ctx context.Context) (*ghRelease, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet,
		"https://api.github.com/repos/{{GITHUB_OWNER}}/{{GITHUB_REPO}}/releases/latest", nil)
	if err != nil {
		return nil, err
	}
